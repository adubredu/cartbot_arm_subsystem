...
while not rospy.is_shutdown():
	...
	if ('take' in command or 'get' in command) and ('container' in command or 'food' in command) and ('microwave' in command):
		...
		# Start move items commands

	if ('move' in command or 'get' in command) and ('item' in command):	
		terminate_thread(task)
		if not env["fridgeOpen"] and not env["holdingSomething"]:
			task = Thread(target=MoveItem, args=(lLimb, rLimb, lGripper, pause_event,), name="moveWaterBottle")
			pause_event.clear()
			task.daemon = True
			task.start()
			print(task.name)
		if env["fridgeOpen"] and not env["holdingSomething"]:
			task = Thread(target=CloseFridgeMoveItem, args=(lLimb, rLimb, lGripper, pause_event,), name="moveWaterBottle")
			pause_event.clear()
			task.daemon = True
			task.start()
			print(task.name)
