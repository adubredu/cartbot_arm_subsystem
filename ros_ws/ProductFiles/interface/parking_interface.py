...
class robot_gui:
	...
	def __init__(self):
		self.standard_x_dist = 0.7
		self.standard_y_dist = 0.9
		self.root = Tk()
		self.root['bg'] = "#ccefff"
	...
	def adjust_robot(self, env):
		...
		if (-0.1 < x_dist - self.standard_x_dist < 0.1) and (-0.1 < y_dist - self.standard_y_dist < 0.1):
			pos = canvas.coords(self.robot)
			d_x = (self.canvas_width/2) - pos[2]
			d_y = (self.canvas_height/2) - pos[3]
			self.canvas.move(self.robot, d_x, d_y)
			return 


		if x_dist - self.standard_x_dist> 0.1:
			delta_x = dist_to_pix(self.standard_x_dist-x_dist)
			self.canvas.move(self.robot, delta_x,0)

		elif x_dist - self.standard_x_dist < 0.1:
			delta_x = dist_to_pix(self.standard_x_dist-x_dist)
			self.canvas.move(self.robot, delta_x, 0)


		if y_dist - self.standard_y_dist> 0.1:
			delta_y = dist_to_pix(self.standard_y_dist-y_dist)
			self.canvas.move(self.robot, 0, delta_y)

		elif y_dist - self.standard_y_dist < 0.1:
			delta_y = dist_to_pix(self.standard_y_dist-y_dist)
			self.canvas.move(self.robot, 0, delta_y)
