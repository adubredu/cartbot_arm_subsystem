...
def pickBottleFromOpenFridge(lLimb,rLimb,gripper, pause_event):
	...
	moveOnAxis(lLimb, 'y', .06, .06, pause_event)
	time.sleep(1)
...
def moveToTableAfterRetrieve(lLimb, rLimb, gripper, pause_event):
	...
	moveOnAxis(lLimb, 'z', -.18, .08, pause_event)
	time.sleep(1)
	waitForNotPause(pause_event)
	gripper.open()
	time.sleep(1)
	moveOnAxis(lLimb, 'z', .08, .04, pause_event)
	moveOnAxis(lLimb, 'x', -.06, .06, pause_event)
...
def openTheMicrowave(lLimb, rLimb, pause_event):
	try:
		sys.path.insert(0, './ProductFiles')
	except IOError as e:
		print(e)

	playPositionFile('openMicrowaveP1.wp', lLimb, rLimb, pause_event)

def closeTheMicrowave(lLimb, rLimb, pause_event):
	try:
		sys.path.insert(0, './ProductFiles')
	except IOError as e:
		print(e)

	playPositionFile('closeMicrowaveP1.wp', lLimb, rLimb, pause_event)

def turnOnMicrowave(lLimb, rLimb, pause_event):
	playPositionFile('turnOnMicrowave.wp', lLimb, rLimb, pause_event)
	moveOnAxis(lLimb, 'z', -.09, .08, pause_event)
	playPositionFile('turnOnMicrowaveP2.wp', lLimb, rLimb, pause_event)

def turnOffMicrowave(lLimb, rLimb, pause_event):
	moveOnAxis(lLimb, 'z', .09, .07, pause_event)
...
def getFoodContainer(lLimb, rLimb, gripper, pause_event):
	...
	moveOnAxis(lLimb, 'y', .05, .04, pause_event)
	time.sleep(.5)
	waitForNotPause(pause_event)
	gripper.close()
	time.sleep(.5)
	moveOnAxis(lLimb, 'z', .05, .04, pause_event)
	moveOnAxis(lLimb, 'y', -.30, .15, pause_event)
...
def placeFoodContainerInMicrowave(lLimb, rLimb, gripper, pause_event):
	...
	moveOnAxis(lLimb, 'z', -.04, .04, pause_event)
	time.sleep(.5)
	waitForNotPause(pause_event)
	gripper.open()
	moveOnAxis(lLimb, 'y', -.2, .1, pause_event)
