...
def heard(recognizer, audio):
	...
	sens = 1
	commands = [("move arm forward", sens), ("move arm backward", sens), ("move arm left", sens), 
                ...
                ("put a water bottle on the table", sens), ("continue", sens), ("cook", sens), 
                ("put food in microwave", sens), ('microwave is open', sens), ('get the food from the microwave', sens)]
...
while not rospy.is_shutdown():
	...
	if lastAliveStatus and not task.is_alive():
		...
		### Check environment variable and the task that just ended ###
	...
	if 'open' in command and 'fridge' in command:
		terminate_thread(task)
	...
	if 'get' in command and 'water bottle' in command:
		...
		if env['fridgeOpen'] and env['hasBottle']:
			pass
		rawCommand = ""
	if 'close' in command and 'fridge' in command:
		terminate_thread(task)
		if env['fridgeOpen']:
